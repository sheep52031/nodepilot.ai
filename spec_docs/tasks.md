...
- [ ] 2.4 修改標註按鈕為「標註困惑」並整合立即處理（W1：UI 工作流程）
  - 修改按鈕文字：「生成教學」→「標註困惑」
...
- [ ] 2.5 實作 Side Panel 困惑列表管理（W1：困惑管理 UI）
  - 建立 Side Panel 困惑列表顯示當前文章的所有標註
  - 實作困惑點詳情顯示：選取文字、困惑描述、音檔轉錄結果
  - 建立困惑點編輯、刪除功能
  - 實作困惑列表即時更新（新增標註後立即顯示）
  - 建立「生成教學」按鈕，用戶主控教學生成時機
  - _需求: 3.1, 3.2_
...
- [ ] 3.3 整合 Whisper API 立即轉錄（W1：即時 AI 轉錄）
